---
name: mnemoport
description: Migrate, export, inspect, plan, install, or undo portable AI assets between Qoder, Codex, Claude Code, and Cursor, including same-tool, cross-device, multi-workspace, and plugin-inventory work. Use when the user asks to move or synchronize instructions, skills, prompts, preferences, MCP definitions, plugins, or extensions.
---

# MnemoPort for Qoder

Treat the detected Qoder CLI, Desktop, or IDE entrypoint as the current host; never reuse a capability conclusion from another entrypoint. Use the `mnemo` CLI as the authority for paths, parsing, package security, conflict handling, writes, and rollback.
...
2. For a local source, run `mnemo inventory --from <platform> --invoked-by qoder --json` first. Report every skipped/manual category; an empty result is not proof that nothing exists.
   ...
   - When plugins or IDE extensions are in scope, explain that the trusted vendor list may create incidental local state, then separately run `mnemo plugin-inventory --from <platform> --workspace <source-path> --json`. Qoder CLI/Desktop/IDE must remain `manual` unless MnemoPort reports an exact admitted structured interface. Never copy Qoder caches or claim entries were packaged, installed, or target-verified.
