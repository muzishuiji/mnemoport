...
The Release workflow can be exercised manually without publishing. A tag matching the workspace version exactly (`vMAJOR.MINOR.PATCH` or a SemVer prerelease such as `v0.1.0-alpha.1`) builds locked release binaries for Linux x86-64, macOS Apple Silicon, and Windows x86-64. A mismatched tag fails before any build begins. A tagged run publishes compressed binaries, SHA-256 checksum files, and a CycloneDX JSON SBOM; tags containing a prerelease suffix create a GitHub prerelease.
